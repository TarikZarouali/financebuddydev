...
(function () {
  ...
  function checkColorTheme() {
    // check if the user previously selected a color theme
    return localStorage.getItem("colorTheme") !== null
...
(function () {
  ...
  var circularProgressBars =
    document.getElementsByClassName("js-c-progress-bar");
...
(function () {
  ...
  Drawer.prototype.initClick = function (event) {
    ...
    if (
      ...
      !event.target.classList.contains("js-drawer")
    )
      return;
    event.preventDefault();
...
(function () {
  ...
  function updateItemsAttributes(filter) {
    ...
    for (var i = 0; i < filter.items.length; i++) {
      ...
      if (
        filter.items[i].classList.contains("is-hidden") &&
        filter.filterList[i]
      ) {
...
(function () {
  var ListFilter = function (element) {
    ...
    this.searchCancel = this.element.getElementsByClassName(
    ...
    this.noResults = this.element.getElementsByClassName(
      "js-list-filter__no-results-msg"
    );
    this.searchTags = [];
    this.resultsNr = this.element.getElementsByClassName(
      "js-list-filter__results-nr"
    );
  ...
  function initListFilter(element) {
    ...
    for (var i = 0; i < element.items.length; i++) {
      var tags = "";
      var label = element.items[i].getElementsByClassName(
        "js-list-filter__label"
      );
      if (label.length > 0) {
    ...
    element.list.addEventListener("click", function (event) {
      var removeBtn = event.target.closest(
        ".js-list-filter__action-btn--remove"
      );
      if (!removeBtn) return;
      event.preventDefault();
  ...
  function filterItems(element, value) {
    ...
    for (var i = 0; i < element.items.length; i++) {
      value == ""
        ? array.push(false)
        : array.push(filterItem(element, i, searchArray));
    }
  ...
  function filterItem(element, index, searchArray) {
    ...
    for (var i = 0; i < searchArray.length; i++) {
      if (
        searchArray[i] != "" &&
        searchArray[i] != " " &&
        element.searchTags[index].toLowerCase().indexOf(searchArray[i]) < 0
      ) {
        found = false;
        break;
      }
  ...
  function updateResultsNr(element) {
    ...
    if (element.resultsNr.length > 0)
      element.resultsNr[0].innerHTML = element.visibleItemsNr;
  }
...
(function () {
  ...
  function setGridLayout(grid) {
    ...
    grid.activeColumns = parseInt(
      (containerWidth + grid.colGap) / (grid.colStartWidth + grid.colGap)
    );
    if (grid.activeColumns == 0) grid.activeColumns = 1;
    grid.colWidth = parseFloat(
      (containerWidth - (grid.activeColumns - 1) * grid.colGap) /
        grid.activeColumns
    );
...
(function () {
  ...
  function resetAppearance(readMore) {
    ...
    for (var i = 0; i < readMore.moreContent.length; i++)
      readMore.moreContent[i].classList.add("is-hidden");
  ...
  function updateVisibility(readMore, visibile) {
    for (var i = 0; i < readMore.moreContent.length; i++)
      readMore.moreContent[i].classList.toggle("is-hidden", !visibile);
    ...
    if (readMore.ellipsis.length > 0)
      readMore.ellipsis[0].classList.toggle("is-hidden", visibile);
...
(function () {
  ...
  function checkRangeSupported(slider) {
    ...
    slider.element.classList.toggle(
...
(function () {
  ...
  if (themeSwitch) {
    ...
    function resetTheme(target) {
      ...
      if (target.checked) {
        htmlElement.removeAttribute("data-theme");
        localStorage.setItem("themeSwitch", "floral");
      }
      if (target.checked) {
        htmlElement.removeAttribute("data-theme");
        localStorage.setItem("themeSwitch", "woody");
      }
      if (target.checked) {
        htmlElement.removeAttribute("data-theme");
        localStorage.setItem("themeSwitch", "water");
      } else {
        htmlElement.removeAttribute("data-theme");
        localStorage.setItem("themeSwitch", "light");
      }
...
(function () {
  ...
  function initSingleToast(obj, toast) {
    ...
    toast.addEventListener("openToast", function () {
      if (!toast.classList.contains("toast--hidden") || obj[id]["closing"])
        return;
...
(function () {
  ...
  function resetLabel(datePicker, index, input, initialLabel) {
    ...
    datePicker.selectedMonth[index] !== false &&
    datePicker.selectedDay[index]
...
(function () {
  var Ddf = function (opts) {
    ...
    this.labelEnd = this.element.getElementsByClassName(
      "js-ddf__files-counter"
    );
    this.labelEndMessage =
      this.labelEnd.length > 0 ? this.labelEnd[0].innerHTML.split("%") : false;
    this.droppedFiles = [];
  ...
  function initDndMessageResponse(element) {
    ...
    if (element.options.showFiles) {
      element.filesList =
        element.element.getElementsByClassName("js-ddf__list");
      if (element.filesList.length == 0) return;
      element.fileItems =
        element.filesList[0].getElementsByClassName("js-ddf__item");
      if (element.fileItems.length > 0)
        element.fileItems[0].classList.add("is-hidden");
      ...
      if (element.options.upload)
        element.progress =
          element.element.getElementsByClassName("js-ddf__progress");
    }
  }
  ...
  function storeDroppedFiles(element, fileData) {
    ...
    element.droppedFiles = element.droppedFiles.concat(
      element.lastDroppedFiles
    );
  ...
  function formatInList(element, index) {
    ...
    for (var i = 0; i < element.options.acceptFile.length; i++) {
      if (
        element.lastDroppedFiles[index].type == element.options.acceptFile[i] ||
        type == element.options.acceptFile[i] ||
        (extension && extension == element.options.acceptFile[i])
      ) {
        accepted = true;
        break;
      }
      ...
      if (
        extension &&
        extensionInList(extension, element.options.acceptFile[i])
      ) {
  ...
  function updateDndAreaMessage(element) {
    if (element.progress && element.progress[0]) {
      ...
      element.progress[0].classList.toggle(
        "is-hidden",
        element.droppedFiles.length == 0
      );
    ...
    if (element.droppedFiles.length > 0 && element.labelEndMessage) {
      ...
      if (element.labelEndMessage.length > 3) {
        finalMessage =
          element.droppedFiles.length > 1
            ? element.labelEndMessage[0] +
              element.labelEndMessage[2] +
              element.labelEndMessage[3]
            : element.labelEndMessage[0] +
              element.labelEndMessage[1] +
              element.labelEndMessage[3];
      }
      element.labelEnd[0].innerHTML = finalMessage.replace(
        "{n}",
        element.droppedFiles.length
      );
    }
  }
  ...
  function updateDndList(element) {
    ...
    for (var i = 0; i < element.lastDroppedFiles.length; i++) {
      clone.getElementsByClassName("js-ddf__file-name")[0].textContent =
        element.lastDroppedFiles[i].name;
    ...
    element.filesList[0].classList.toggle(
      "is-hidden",
      element.droppedFiles.length == 0
    );
  }
  ...
  function initRemoveFile(element) {
    ...
    element.filesList[0].addEventListener("click", function (event) {
      ...
      var removedFile = element.droppedFiles.splice(
        element.droppedFiles.length - index,
        1
      );
      if (
        element.progress &&
        element.progress.length > element.droppedFiles.length - index
      ) {
      ...
      if (
        lastDroppedIndex >= 0 &&
        lastDroppedIndex < element.lastDroppedFiles.length - 1
      ) {
        element.lastDroppedFiles.splice(
          element.lastDroppedFiles.length - index,
          1
        );
      }
      item.remove();
  ...
  function storeMultipleProgress(element) {
    ...
    for (var i = 0; i < element.lastDroppedFiles.length; i++) {
      var progress =
        element.fileItems[
          element.droppedFiles.length - delta - i
        ].getElementsByClassName("js-ddf__progress")[0];
  ...
  function initProgress(element, start, end, bool) {
    ...
    for (var i = start; i < end; i++) {
      (function (i) {
        ...
        element.progress[i].addEventListener(
          "updateProgress",
          function (event) {
            if (event.detail.value == 100)
              element.progress[i].classList.add(element.progressCompleteClass);
          }
        );
      })(i);
    }
...
(function () {
  ...
  if (flexHeader.length > 0) {
    ...
    linksList.addEventListener("click", function (event) {
      if (
        !event.target.closest(".js-f-header__link") ||
...
(function () {
  function MdEditor(element, actions) {
    this.element = element;
    this.textarea = this.element.getElementsByClassName(
      "js-md-editor__content"
    );
    this.actionsWrapper = this.element.getElementsByClassName(
      "js-md-editor__actions"
    );
  ...
  function initMdEditor(element) {
    if (element.textarea.length < 1 || element.actionsWrapper.length < 1)
      return;
  ...
  function updateTextareaSelection(element) {
    ...
    element.selectionContent = element.textarea[0].value.slice(
      element.selectionStart,
      element.selectionEnd
    );
  }

  function getCode(element, action) {
    // get content to insert in the textarea
    ...
    if (actionInfo.content.indexOf("content") < 0) {
      ...
      element.selectionStart =
        element.selectionStart + element.selectionContent.length;
      element.selectionContent = "";
    }
    var newContent = actionInfo.content.replace(
      "content",
      element.selectionContent
    );
  ...
  function replaceSelectedText(element, text) {
    ...
    element.textarea[0].selectionEnd =
      element.selectionEnd -
      element.selectionContent.length +
      text.length -
      element.actionEndLength;
    if (element.selectionStart != element.selectionEnd) {
      element.textarea[0].selectionStart =
        element.textarea[0].selectionEnd - element.selectionContent.length;
    } else {
  ...
  function addNewLine(element, newLine) {
    ...
    if (element.selectionStart > 0) {
      ...
      var previousChar = element.textarea[0].value.slice(
      ...
      menu.moreItemsTrigger[0].addEventListener("keydown", function (event) {
        if (
          ...
          menu.menuInstance.toggleMenu(
            !menu.subMenu.classList.contains("menu--is-visible"),
            true
          );
        }
      });
  ...
  function navigateItems(list, event, direction, prevIndex) {
    ...
    var index =
        typeof prevIndex !== "undefined"
          ? prevIndex
          : Array.prototype.indexOf.call(list, event.target),
      nextIndex = direction == "next" ? index + 1 : index - 1;
...
(function () {
  var MultiCustomSelectTwo = function (element) {
    this.element = element;
    this.checkboxes = this.element.getElementsByClassName(
      "js-multi-select-v2__input"
    );
  ...
  function initMultiCustomSelectTwo(element) {
    ...
    element.element.addEventListener("change", function (event) {
      ...
      if (label)
        Util.toggleClass(label, element.checkedClass, event.target.checked);
      resetCounter(element);
    });
  ...
  function resetCheckedClasses(element) {
    var checkedLabels = element.element.getElementsByClassName(
      element.checkedClass
    );
  ...
  if (customSelect.length > 0) {
    for (var i = 0; i < customSelect.length; i++) {
      (function (i) {
        new MultiCustomSelectTwo(customSelect[i]);
      })(i);
    }
  }
})();
...
(function () {
  var PasswordStrength = function (opts) {
    ...
    this.element = this.options.element;
    this.input = this.element.getElementsByClassName(
      "js-password-strength__input"
    );
    this.reqs = this.element.getElementsByClassName(
      "js-password-strength__req"
    );
    this.strengthSection = this.element.getElementsByClassName(
      "js-password-strength__meter-wrapper"
    );
    this.strengthValue = this.element.getElementsByClassName(
      "js-password-strength__value"
    );
    this.strengthMeter = this.element.getElementsByClassName(
      "js-password-strength__meter"
    );
  ...
  function initInput(password) {
    ...
    password.input[0].addEventListener("blur", function cb(event) {
      ...
      for (var i = 0; i < password.reqs.length; i++) {
        if (!Util.hasClass(password.reqs[i], password.reqMetClass))
          Util.addClass(password.reqs[i], password.reqNoMetClass);
      }
    });
  }
  ...
  function toggleCheckStrength(password) {
    ...
    Util.toggleClass(
      password.strengthSection[0],
      "is-hidden",
      password.input[0].value.length == 0
    );
  }
  ...
  function checkStrength(password) {
    ...
    if (password.strengthValue.length > 0) {
      ...
      if (response.score >= 1)
        password.strengthValue[0].textContent =
          password.strengthLabels[response.score - 1];
    ...
    if (password.strengthMeter.length > 0) {
      ...
      password.strengthMeter[0].firstElementChild.style.width =
        score / 0.04 + "%";
      removeStrengthClasses(password);
      if (response.score >= 1)
        Util.addClass(
          password.strengthMeter[0],
          "password-strength__meter--fill-" + response.score
        );
      else
        Util.addClass(
          password.strengthMeter[0],
  ...
  function checkConditions(password) {
    ...
    for (var i = 0; i < password.reqs.length; i++) {
      ...
      if (password.passwordInteracted)
        Util.toggleClass(password.reqs[i], password.reqNoMetClass, !result);
    }
  }
  ...
  function checkSingleCondition(value, req) {
    ...
    switch (true) {
      ...
      case req.indexOf("length:") == 0:
        ...
        if (reqArray.length > 2 && result)
          result = value.length <= parseInt(reqArray[2]);
        break;
  ...
  function removeStrengthClasses(password) {
    var classes = password.strengthMeter[0].className
      .split(" ")
      .filter(function (c) {
  ...
  var passwordStrength = document.getElementsByClassName(
...
(function () {
  ...
  function createSingleBtn(link) {
    ...
    var btnClasses = link
      .getAttribute("class")
      .replace("js-sidenav-v4__link", "js-sidenav-v4__btn");
    ...
    if (!Util.hasClass(listItem, "sidenav-v4__item--current"))
      Util.addClass(listItem, "sidenav-v4__item--collapsed");
  }
  ...
  function toggleSubLists(nav) {
    ...
    nav.addEventListener("click", function (event) {
      ...
      Util.toggleClass(
        ...
        !Util.hasClass(btn.parentElement, "sidenav-v4__item--collapsed")
      );
    });
  }
...
(function () {
  ...
  function toggleMsClass(slider) {
    ...
    if (imeAlignSuport || !cssVariablesSupport)
      Util.addClass(slider.element, "slider--ms-fallback"); // IE and Edge (<=18) Fallback
  }
      ...
      var intervalId = setInterval(function () {
        ...
        progressBar.dispatchEvent(
          new CustomEvent("updateProgress", {
            detail: { value: progress, duration: 80 },
          })
        );
      }, 100);
    }
...
(function () {
  ...
  function toggleRowSelection(table) {
    ...
    for (var i = 0; i < table.selectRow.length; i++) {
      ...
      table.selectRow[i]
        .closest(".int-table__row")
        .classList.toggle("int-table__row--checked", status);
    }
  ...
  function sortColumns(table, selectedCol, customOrder) {
    ...
    for (var i = 0; i < table.headerCols.length; i++) {
      table.headerCols[i].classList.remove(
        "int-table__cell--asc",
