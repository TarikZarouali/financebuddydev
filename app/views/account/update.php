...
<div class="container max-width-lg padding-y-lg">
    ...
    <form method="POST" action="<?= URLROOT ?>account/update/<?= $data['account']->accountId ?>" onsubmit="updateAccount(event, '<?= $data['account']->accountId ?>')">
    ...
    <div class="dialog__content max-width-xxs" role="alertdialog" aria-labelledby="dialog-title-1" aria-describedby="dialog-description">
        ...
        <footer class="margin-top-md">
            <div class="flex justify-end gap-xs flex-wrap">
                ...
                <a class="btn btn--accent" onclick="handleToastOnCrud('deleteAccount',true)" href="<?= URLROOT; ?>account/delete/<?= $data['account']->accountId ?>">Confirm</a>
            </div>
        </footer>
    </div>
</div>
